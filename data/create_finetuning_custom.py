...
def get_contours(path_json,path_dcm):
    """
    args:
    path_json: path to contour json file
    path_dcm: path to dicom file
    """
    contour_dict = {}
    ...
    for series in dir_series:
        ...
        for file_label in list(label_files):
            ...
            if len(np.unique(lbl))<6:
                continue
            contour_dict = get_contours(path_json,path_dcm)
...
file_name = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_train.tsv")
print("creating ", file_name)
writer = open(file_name, 'w')
writer.writelines(combined_train_data)
writer.close()

file_name_test = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_test.tsv")
print("creating ", file_name_test)
writer = open(file_name_test, 'w')
writer.writelines(combined_test_data)
writer.close()

file_name_val = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_val.tsv")
