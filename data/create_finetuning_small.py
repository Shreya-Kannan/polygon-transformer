...
def get_contours(path_json,path_dcm,path_lbl):
    """
    args:
    path_json: path to contour json file
    path_dcm: path to dcm file
    """
    contour = []
    img = dicom.dcmread(path_dcm).pixel_array
    ...
    for i in range(6):
      try:
          ...
          contours_x.extend([p1_x])
          contours_y.extend([p1_y])
...
dir_dcm = '/home/shreya/scratch/Regional/MRI/'
...
for subdir in subdirs:
    ...
    for series in dir_series:
        ...
        for file_label in list(label_files):
            ...
            path_dcm = os.path.join(dir_dcm, subdir, series, file_label.replace('.npz', '.dcm'))
            ...
            if len(np.unique(lbl))<6:
                continue
            contour, center = get_contours(path_json,path_dcm,path_label)
            ...
            instance = '\t'.join(
                [uniq_id, subdir, 'Generate the correct Regional points', box_string, pts_string, path_dcm, path_label,
                    pts_string]) + '\n'
...
file_name = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_small_train.tsv")
print("creating ", file_name)
writer = open(file_name, 'w')
writer.writelines(combined_train_data)
writer.close()

file_name_test = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_small_test.tsv")
print("creating ", file_name_test)
writer = open(file_name_test, 'w')
writer.writelines(combined_test_data)
writer.close()

file_name_val = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_small_val.tsv")
