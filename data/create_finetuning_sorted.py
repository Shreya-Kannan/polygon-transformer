    ...
    for series in dir_series:
        ...
        for file_label in list(label_files):
            ...
            if len(np.unique(lbl))<6:
                continue
            contour_dict, center = get_contours(path_json,path_dcm,path_label)
            ...
            for key in contour_dict.keys():
                ...
                instance = '\t'.join(
                    [uniq_id, subdir, sentence_dict[key], box_string, pts_string, path_dcm, path_label,
...
file_name = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_sorted_train.tsv")
print("creating ", file_name)
writer = open(file_name, 'w')
writer.writelines(combined_train_data)
writer.close()

file_name_test = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_sorted_test.tsv")
print("creating ", file_name_test)
writer = open(file_name_test, 'w')
writer.writelines(combined_test_data)
writer.close()

file_name_val = os.path.join("/home/shreya/scratch/Regional/polygon-transformer/datasets/finetune/Regional_sorted_val.tsv")
