...
#colours for the regional segments
colorsCbfRainbowRGB = {
  1: [64,67,153],
  2: [72,139,194],
  3: [107,178,140],
...
class RefcocoDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if len(data) == 7:
            uniq_id, path_dcm, path_label, text, poly_original, region_coord, poly_interpolated = data
            train = True
        else:
            uniq_id, path_dcm, path_label, text, poly, region_coord = data
            train = False
        ...
        imgb = image.copy()
        for i in range(1,7):
            imgr = np.where(image == i, colorsCbfRainbowRGB[i][0], imgr)
            imgg = np.where(image == i, colorsCbfRainbowRGB[i][1], imgg)
            imgb = np.where(image == i, colorsCbfRainbowRGB[i][2], imgb)
